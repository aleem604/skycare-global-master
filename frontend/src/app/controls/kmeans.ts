...
// Add process.nextTick(callback) to browser window context
(function(window, nextTick, process, prefixes, i, p, fnc) {
    p = window[process] || (window[process] = {});
    while (!fnc && i < prefixes.length) {
        fnc = window[prefixes[i++] + 'equestAnimationFrame'];
    }
    p[nextTick] = p[nextTick] || (fnc && fnc.bind(window)) || setImmediate || setTimeout;
...
export class Group {
    ...
    public centroidMoved : boolean = true;
    public centroid : any[] = [];
    public centroidOld : any[] = [];
    public centroidIndex : any;
    public cluster : any[] = [];
